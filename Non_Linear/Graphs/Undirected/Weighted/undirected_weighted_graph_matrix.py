...
class UndirectedWeightedGraphMatrix:
    ...
    def __init__(self, num_vertices: int):
        """
        Initialize an adjacency matrix for an undirected weighted graph.
        ...
        for i in range(num_vertices):
            ...
            for j in range(num_vertices):
                row.append(0 if i == j else float('inf'))
            self.adj.append(row)
    ...
    def remove_edge(self, u: int, v: int):
        ...
        self.adj.get(v).set(u, float('inf'))
    ...
    def edges(self):
        """
        Return a list of all edges (u, v, weight), avoiding duplicates.
        ...
        for u in range(self.n):
            row = self.adj.get(u)
            for v in range(u + 1, self.n):  # Avoid double-counting
                weight = row.get(v)
                if weight != float('inf'):
    ...
    def __repr__(self):
        return f"UndirectedWeightedGraphMatrix(num_vertices={self.n})"


# ------------------- TEST CODE -------------------
if __name__ == "__main__":
    g = UndirectedWeightedGraphMatrix(4)
    g.add_edge(0, 1, 5)
    g.add_edge(0, 2, 10)
    g.add_edge(1, 3, 2)
    g.add_edge(2, 3, 1)
